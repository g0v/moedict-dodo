  ...
  $(function(){
    return $.get("moetris.txt", null, function(ALL){
      ...
      function pick(cur){
        ...
        cur == null && (cur = '');
        if (!/[^＊？]/.test(cur)) {
          return "的不一我是人有了大國來生在子們中上他時小地出以學可自這會成家到為天心年然要得說過個著能下動發臺麼車那行經去好開現就作後多方如事公看也長面力起裡高用業你因而分市於道外沒無同法前水電民對兒日之文當教新意情所實工全定美理頭本明氣進樣都主間老想重體山物知手相回性果政只此代和活媽親化加影什身己灣機部常見其正世"[Math.floor(Math.random() * 150)];
        }
        if (Math.random() < 0.05) {
          return "＊";
        }
